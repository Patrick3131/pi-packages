...
function parseCookieHeader(cookieHeader: string): AuthCookie[] {
  return cookieHeader
    ...
    .map((part) => {
      ...
      return eqIndex === -1
        ? null
        : { name: part.slice(0, eqIndex).trim(), value: part.slice(eqIndex + 1).trim() };
    })
...
function parseCookies(value: AuthProfileConfig["cookies"]): AuthCookie[] | undefined {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed
        .filter(
          (cookie): cookie is AuthCookie =>
            Boolean(cookie && typeof cookie === "object" && "name" in cookie && "value" in cookie)
        )
